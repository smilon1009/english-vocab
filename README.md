...
> **当前执行策略（2026-09-28 起）：** 到期复习优先，默认每天最多 5 个新词，只启用 level 1–2 高频词；每周精听 3 次、短文阅读 2 次、五句话写作 3 次、口述录音 1 次。详细安排见 [`plan/12周英语执行计划_2026-09-28.md`](./plan/12周英语执行计划_2026-09-28.md)。已有账号的云端个人计划不会被仓库模板自动覆盖，请以此处和 App 设置为准。

> IELTS 没有官方封闭的“全部单词”清单。当前 2125 张卡片属于待逐条核验的旧词库；仓库已用 NGSL（通用高频）和 NAWL（学术）建立可追溯覆盖基准。现有词头覆盖率分别为 52.1% 和 22.0%，缺失清单在 `data/coverage_report.json`，审核合格后才会分批进入工作台。

> 工作台只负责词汇记忆。听、说、读、写、语法和后续 IELTS 专项的完整安排见 [`plan/完整英语学习系统.md`](./plan/完整英语学习系统.md)；使用 CodeBuddy 时可复制 [`plan/CodeBuddy英语教练开场.md`](./plan/CodeBuddy英语教练开场.md)。
...
工作台负责“每天怎么背”，计划文档负责“为什么这么背、学到什么程度、什么时候检查”。当前优先保证可持续，不追赶中断期间的新词数量。
...
- 顶部显示雅思检查点倒计时与词汇进度；下方记录精听跟读、词汇工作台和五句话日记。默认跟读 10–15 分钟，日记每周 3 次。
...
- 这个 GitHub 仓库只放网页源码、学习卡片和开放许可的参考词表，**不含任何用户学习数据**。
...
│   ├── words.js              #   旧基础词库（Level 1–4，待逐条核验）
│   ├── words_ielts1.js       #   旧词包①（文件名保留以兼容用户进度）
│   ├── words_ielts2.js       #   旧词包②
│   ├── words_ielts3.js       #   旧词包③
│   ├── words_ielts4.js       #   旧词包④
│   ├── words_ielts5.js       #   旧词包⑤
...
│
├── data/
│   ├── coverage_report.json  # NGSL/NAWL 覆盖结果和缺失词头审核队列
│   └── reference/            # 开放许可的 NGSL/NAWL 参考词表及许可证
│
├── plan/                     # 12 周英语计划、每日执行、学习方法与词库质量规则
│
├── pwa/                      # 手机「添加到主屏幕」+ 离线缓存
...
│
└── tools/                    # 启动器和词库审计脚本
    ├── audit_words.mjs       #   检查词条结构、重复和可疑词形
    ├── audit_coverage.mjs    #   生成 NGSL/NAWL 覆盖报告
...
- **应用逻辑**（`<script>` 内约 900 行）：
  ...
  - **三态判词**：判词支持「认识 / 😐半熟 / 不认识」；半熟词直接入已学会但 3 天后回炉再确认，避免一次判死
  ...
  - **坚持"输出型"学习（无快速放水模式）**：判词判生词与全部复习都要求**自己拼写整词**——刻意练习"能默写"才是真掌握；为此不设"认识/不认识"一键放水
  - **半熟快速回炉 3 天**：判词标「半熟」的词 3 天后回到「可回炉」再确认（原 7 天），缩小遗忘窗口
  - **睡前速记引导**：今日生词全部学完时，庆祝卡提示"睡前速记 → 复习当天"（仍是拼写输出）；「复习到期」无到期时也显示该入口
  - **当日错词补刀**：当天任何复习答错的词自动记录，复习完成页出现"⚔️ 当日错词补刀"入口，答对即清、答错留待明日队列
  - **词库质量优先**：当前约 **2125 词**（words.js + 旧词包①～⑨），全部视为待逐条核验。NGSL/NAWL 只作覆盖基准，缺失词审核合格后分批加入；工作台支持可选 `answer` 字段处理 `blew/studies` 等正确词形。
...
| `js/words_ielts1.js`～`js/words_ielts9.js` | 历史词包，文件名与 `pack` 标识保留以兼容已有云端进度；内容均视为待逐条核验 |
...
| 文件 | 作用 |
|---|---|
| `tools/audit_words.mjs` | 检查必填字段、重复词、例句填空和可疑词形 |
| `tools/audit_coverage.mjs` | 对照 NGSL/NAWL 生成覆盖率和缺失词头队列 |
| `tools/check_app_syntax.mjs` | 检查 `index.html` 内联脚本语法 |
...
- **词库机制**：词包文件按 `WORDS.push` 追加并自动去重；当前库约 2125 词，先做质量审计；NGSL/NAWL 参考层提供可追溯覆盖报告；判词跨词包轮转随机抽取；词条支持 `syn/root/answer` 可选字段
...
本仓库 v3 起改为账号云同步、从零开始记录进度，不再依赖本地 node 服务。老版本工作台所在的 `09_english` 文件夹已从个人 study 仓库删除；英语学习的计划与日记现已整理到本仓库 [`plan/`](./plan/README.md)。
