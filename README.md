...
│   ├── words_ielts6.js       #   雅思核心词包⑥ —— 日常高频补全（第四次，level1 为主）
│   ├── words_ielts7.js       #   雅思核心词包⑦ —— 进阶高频补全（第五次，level2 为主）
│   ├── words_ielts8.js       #   雅思核心词包⑧ —— 通用主题扩展（第六次；首批含 syn/root 字段）
...
- **应用逻辑**（`<script>` 内约 900 行）：
  - 判词（出英文→确认中文→认识/不认识；**抽词随机**：每个词包先打乱再轮转合并，新词不会集中在某一个词库文件）
  ...
  - **同义词/词根可选展示**：词条可带 `syn[]`（同义词）与 `root`（词根拆解），在判词揭晓卡与生词本详情里以小字展示，不干扰学习主流程（数据随扩容逐步补充）
  - **词库扩容计划**：当前约 **2025 词**（words.js 500 + 雅思包①~⑧）。日常高频(level1)已基本补全，level2 高频持续推进中；目标雅思 ~6000。判词抽词已改为**跨词包轮转随机**，任何文件/词包都不会在每日任务里成批扎堆。后续 `words_ielts9/10/…` 继续。逻辑无需改动：`index.html` 逐个引入、按 `WORDS` 全局数组自动并入去重，新词包自动默认启用、可手动关停
...
| `js/words_ielts5.js` | 雅思核心词包⑤，同上，`pack:"ielts5"` + `level` 权重（学术/拓展扩容，第三次） |
| `js/words_ielts6.js` | 雅思核心词包⑥，同上，`pack:"ielts6"` + `level` 权重（日常高频补全，第四次） |
| `js/words_ielts7.js` | 雅思核心词包⑦，同上，`pack:"ielts7"` + `level` 权重（进阶高频补全，第五次） |
| `js/words_ielts8.js` | 雅思核心词包⑧，同上，`pack:"ielts8"` + `level` 权重（通用主题扩展，第六次；已开始带可选 `syn[]` 同义词与 `root` 词根字段） |
...
- **词库扩容机制**：词包文件按 `WORDS.push` 追加、自动去重合并，扩词不改任何业务逻辑；当前库约 2025 词、分批发往 ~6000；判词跨词包轮转随机抽取；词条可选 `syn/root` 扩展字段已接入展示
