...
- **应用逻辑**（`<script>` 内约 900 行）：
  ...
  - **词库扩容计划**：当前约 **1948 词**（words.js 500 + 雅思包①~⑦）。日常高频(level1)已基本补全，level2 高频持续推进中；目标雅思 ~6000。判词抽词已改为**跨词包轮转随机**，任何文件/词包都不会在每日任务里成批扎堆。后续 `words_ielts8/9/…` 继续。逻辑无需改动：`index.html` 逐个引入、按 `WORDS` 全局数组自动并入去重，新词包自动默认启用、可手动关停
...
| `js/words_ielts7.js` | 雅思核心词包⑦，同上，`pack:"ielts7"` + `level` 权重（进阶高频补全，第五次；后续 words_ielts8… 同格式追加） |
...
- **词库扩容机制**：词包文件按 `WORDS.push` 追加、自动去重合并，扩词不改任何业务逻辑；当前库约 1948 词、分批发往 ~6000；判词跨词包轮转随机抽取
